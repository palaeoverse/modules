...
List items can appear incrementally with by wrapping the list in `::: {.incremental}`:
...
In a code block, we can use `code-line-numbers` to highlight lines of code (note that in this case we need to use `.r` instead of `r` in the block).

Using `code-line-numbers="1|2-5|6|"` will highlight the first line, then lines 2-5, then line 6, and finally the entire block:

<br>

```{.r code-line-numbers="1|2-5|6|"}
x <- 1
y <- paste(
  "hello there",
  "how are you?"
