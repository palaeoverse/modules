...
# default path name (see sync_filter.r)
filter_copy  <- "web_and_slides_autogenerated.lua"
filter_path  <- if (length(filter_arg)) str_remove(filter_arg[[1]], "^--filter=") else NA_character_
...
# --- add the Lua filter to front matter (relative to the output doc) ---------
inject_filter <- function(yaml, filter_path, output) {
  ...
  rel <- if (is.na(filter_path)) filter_copy
         else path_rel(path_abs(filter_path), start = path_dir(path_abs(output)))
  ...
  names_re <- "web_and_slides(_autogenerated)?\\.lua"
  ...
  if (is.na(fi)) {
    ...
    warning("Inline `filters: [...]` found; add ", rel, " at pre-ast manually"); yaml
  } else {
